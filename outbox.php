			  ...
			  <div class="box box-solid">
				...
				<div class="box-body no-padding">
				  <ul class="nav nav-pills nav-stacked">
					...
					<li class="active"><a href="outbox.php"><i class="fa fa-envelope"></i> Outbox
...
<?php		
		if(count($_subject_o) != 0){
			echo	'<div class="btn-group">
						<!-- Check all button -->
						<button type="button" class="btn btn-default btn-sm checkbox-toggle"><i class="fa fa-square-o"></i></button>
						<button type="submit" name="delete_" class="btn btn-default btn-sm" title="Delete"><i class="fa fa-trash-o"></i></button>
					</div>';
		}
?>
					</div>
...
<?php
			...
			for($i=0;$i<count($_subject_o);$i++){
					echo  '<tr>
							<td><input type="checkbox" class="addAttr_'.$i.'" value="'.$_inbox_pk_o[$i].'"></td>
							<td class="mailbox-name"><a href="read-sms.php?id='.$_inbox_pk_o[$i].'">'.$_subject_o[$i].'</a></td>
							<td class="mailbox-date">'.$_time_o[$i].'</td>
						  </tr>';
			}
?>						  
						  </tbody>
